...
// ==========================================
export const submitDustbinRequest = functions.https.onCall(async (data, context) => {
  ...
  if (!citizenDoc.exists || citizenDoc.data()?.role !== 'citizen') {
  ...
  const newRequest = {
    ...data,
    citizenId,
    citizenName: citizenDoc.data()?.name,
    status: "ASSIGNED", // Initial status
    assignedVillageStaffId,
...
// ==========================================
export const approveDustbinRequest = functions.https.onCall(async (data, context) => {
...
// ==========================================
export const completeDustbinRequest = functions.https.onCall(async (data, context) => {
    if (!context.auth || await getUserRole(context.auth.uid) !== 'village_staff') {
...
// ==========================================
export const acceptCleaningTask = functions.https.onCall(async (data, context) => {
...
// ==========================================
export const completeCleaningTask = functions.https.onCall(async (data, context) => {
    if (!context.auth || await getUserRole(context.auth.uid) !== 'cleaning_staff') {
...
// ==========================================
export const resolveIssueByAdmin = functions.https.onCall(async (data, context) => {
